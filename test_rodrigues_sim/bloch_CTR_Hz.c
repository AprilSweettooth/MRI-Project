...
int blochsim(double *b1real, double *b1imag, 
		...
		double *mx, double *my, double *mz, int mode, double *spoil, double *DeltaF)
...
for (tcount = 0; tcount < ntime; tcount++)
	...
	// LV: Frequency offset of the excitation pulse
	if (*DeltaF++ != 0)
	{
		df = df - *DeltaF++;
	}
...
for (fcount=0; fcount < nfreq; fcount++)
	...
	if (mode == 3)	/* Steady state AND record all time points. */
		...
		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			...
			*dzptr, mx, my, mz, 1, spoil, DeltaF);

		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			e1, e2, *dfreq, *dxptr++, *dyptr++, 
			*dzptr++, mx, my, mz, 2, spoil, DeltaF);
		}
	else
		{
		blochsim(b1real, b1imag, xgrad, ygrad, zgrad, tsteps, ntime, 
			e1, e2, *dfreq, *dxptr++, *dyptr++, 
			*dzptr++, mx, my, mz, mode, spoil, DeltaF);
		}
...
double *spoil; //WTC: Vector size of ti which resets the transverse magnetisation to 0 when spoil == 1.
double *DeltaF; //LV: Vector size of ti which sets the excitation frequency offset
...
if ( (nrhs > 10) &&	 
	...
	if (nrhs > 10) 	 /* Magnetization given, but wrong size! */
		{
		mexErrMsgIdAndTxt("bloch:BadMagnetization","Initial magnetization passed, but not Npositions x Nfreq.");
		}
...
// WTC:Spoiler vector
if ((nrhs > 11) && (mxGetM(prhs[11]) * mxGetN(prhs[1]) == ntime))
{
    spoil = mxGetPr(prhs[11]);
    DEBUG_printf("Spoiler length same as B1 length\n");

}
else if ((nrhs > 11) && (mxGetM(prhs[11]) * mxGetN(prhs[1]) != ntime))
{
...
//LV: Delta frequency for excitation

if ((nrhs > 12) && (mxGetM(prhs[12])*mxGetN(prhs[1])==ntime))
{
	DeltaF = mxGetPr(prhs[12]);
	DEBUG_printf("Offset length same as B1 length\n");

}
else if ((nrhs > 12) && (mxGetM(prhs[12]) * mxGetN(prhs[1]) != ntime))
{
    mexErrMsgIdAndTxt("bloch:BadOffset","Offset length differs from B1 length.");
}
else
{
    	
#ifdef DEBUG
    DEBUG_printf("Assigning excitation offset.\n");
#endif
    DeltaF = (double *)malloc(ntime * sizeof(double));
    for (count=0; count < ntime; count++)
        DeltaF[count]=0.0;
...
blochsimfz(b1r,b1i,gx,gy,gz,tp,ntime,t1,t2,df,nf,dx,dy,dz,npos,mx,my,mz,md,spoil,DeltaF);
...
if (gzaflag==1)
	free(gz);
if (spoilflag==1)
    free(spoil); /* We allocate this if it isn't input*/
